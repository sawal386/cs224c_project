...
def estimate_text_distribution(human_source_path, ai_source_path,save_file_path="Word.parquet",
                              min_ai=3, min_human=5, min_sentence=1):
    """
    ...
    human_data=human_data[human_data['human_sentence'].apply(len) > min_sentence]
    ai_data=ai_data[ai_data['ai_sentence'].apply(len) > min_sentence]
    ...
    frequent_human_words = filter_frequent_words(human_word_counts, min_human)
    frequent_ai_words = filter_frequent_words(ai_word_counts, min_ai)
    frequent_common_vocab = common_vocab.intersection(frequent_human_words.keys(),
                                                      frequent_ai_words.keys())
